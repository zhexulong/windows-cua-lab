...
export function buildGenericPlannerInstruction(params: { targetApp: string; task: string; plannerContext?: GenericPlannerContext; rejectionReason?: string }): string {
  return [
    ...
    '{"summary":"...","action":{"kind":"click|double_click|type|hotkey|drag","target":"string","button":"left|right|middle","position":{"x":number,"y":number},"text":"string","keys":["CTRL","S"],"from":{"x":number,"y":number},"to":{"x":number,"y":number}}}',
    ...
    'Avoid unrelated global app-level actions or hotkeys that do not continue the same entry goal.',
